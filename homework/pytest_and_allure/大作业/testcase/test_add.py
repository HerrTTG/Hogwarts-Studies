...
class Test_add():
    ...
    def test_raise_add(self, name, volume, power, objectget):
        '''
        ...
        if (isinstance(volume, int) and isinstance(power, int)
            ...
            logging.info(
                f'成功添加正确的数据，跳过第二步校验:hero_name:{name} hero_volume:{volume} hero_power:{power}')
            pass
        else:
                ...
                try:
                    ...
                    if name is None:
                        logging.error(
                            ...
                            f'发现问题:新增英雄时如果name传参为null可以正常添加')
                    elif isinstance(volume, float) or isinstance(power, float):
                        logging.error(
                            f'测试步骤二断言结果不正确:添加英雄非正确输入失败用例，hero_name:{name} hero_volume:{volume} hero_power:{power}\n'
                            f'发现问题:新增英雄血量和攻击力并非只支持正整数,但实际可以使用浮点数。')
                    raise
