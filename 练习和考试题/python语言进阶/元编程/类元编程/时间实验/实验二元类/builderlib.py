...
class Builder:  # <1>一个子类构造器
    print('@ Builder body')

    def __init_subclass__(subclass):  # <2>
        print(f'@ Builder.__init_subclass__({subclass!r})')

        def inner_0(self):  # <3>实验的一个实例方法
            print(f'@ SuperA.__init_subclass__:inner_0({self!r})')

        # 通过下面代码加入到子类中
        subclass.method_a = inner_0
...
class Descriptor:  # <1>描述符类
    print('@ Descriptor body')

    def __init__(self):  # <2>演示描述符实例合适创建
        print(f'@ Descriptor.__init__({self!r})')

    def __set_name__(self, owner, name):  # <3>以及在构造属性名绑定合适运行
